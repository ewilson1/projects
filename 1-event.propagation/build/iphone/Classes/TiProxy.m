...
//Rare exceptions to indicate a bug in the _eventpropagation code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_eventpropagation = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_eventpropagation]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.eventpropagation.module"
