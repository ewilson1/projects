...
NSString * k2_layoutmodes$ModuleRequireFormat = @"(function(exports){"
...
@implementation k2_layoutmodesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k2_layoutmodes);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k2_layoutmodes global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k2_layoutmodes" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k2_layoutmodes = [[k2_layoutmodesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k2_layoutmodes];
	
	NSString *_k2_layoutmodesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k2_layoutmodesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k2_layoutmodes valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k2_layoutmodes$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k2_layoutmodes.kroll" 
