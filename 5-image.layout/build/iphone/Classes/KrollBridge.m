...
NSString * k5_imagelayout$ModuleRequireFormat = @"(function(exports){"
...
@implementation k5_imagelayoutObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k5_imagelayout);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k5_imagelayout global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k5_imagelayout" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k5_imagelayout = [[k5_imagelayoutObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k5_imagelayout];
	
	NSString *_k5_imagelayoutNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k5_imagelayoutNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k5_imagelayout valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k5_imagelayout$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k5_imagelayout.kroll" 
