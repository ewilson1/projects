...
NSString * k7_puttingitalltogether$ModuleRequireFormat = @"(function(exports){"
...
@implementation k7_puttingitalltogetherObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k7_puttingitalltogether);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k7_puttingitalltogether global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k7_puttingitalltogether" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k7_puttingitalltogether = [[k7_puttingitalltogetherObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k7_puttingitalltogether];
	
	NSString *_k7_puttingitalltogetherNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k7_puttingitalltogetherNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k7_puttingitalltogether valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k7_puttingitalltogether$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k7_puttingitalltogether.kroll" 
