...
//Rare exceptions to indicate a bug in the _windows code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_windows = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_windows]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.windows.module"
