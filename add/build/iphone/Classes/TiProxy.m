...
//Rare exceptions to indicate a bug in the _add code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_add = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_add]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.add.module"
