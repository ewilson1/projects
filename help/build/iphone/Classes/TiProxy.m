...
//Rare exceptions to indicate a bug in the _help code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_help = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_help]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.help.module"
