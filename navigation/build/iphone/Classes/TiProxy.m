...
//Rare exceptions to indicate a bug in the _navigation code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_navigation = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_navigation]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.navigation.module"
