...
//Rare exceptions to indicate a bug in the _seperate code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_seperate = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_seperate]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.seperate.module"
