...
//Rare exceptions to indicate a bug in the _teas code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_teas = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_teas]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.teas.module"
