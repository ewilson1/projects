...
NSString * wilson_eric_project_1$ModuleRequireFormat = @"(function(exports){"
...
@implementation wilson_eric_project_1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_wilson_eric_project_1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create wilson_eric_project_1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "wilson_eric_project_1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_wilson_eric_project_1 = [[wilson_eric_project_1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_wilson_eric_project_1];
	
	NSString *_wilson_eric_project_1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _wilson_eric_project_1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_wilson_eric_project_1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:wilson_eric_project_1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.wilson_eric_project_1.kroll" 
