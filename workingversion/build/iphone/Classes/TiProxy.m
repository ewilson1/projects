...
//Rare exceptions to indicate a bug in the _workingversion code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_workingversion = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_workingversion]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.workingversion.module"
